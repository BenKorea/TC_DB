...
refer_na_before<-tx_patient_data %>% filter (is.na(refer))
...
refer_na_after<-tx_patient_data %>% filter (is.na(refer))

tx_patient_followup_data <- tx_patient_data %>% filter(!is.na(last_t4))
tx_patient_no_followup_data <- tx_patient_data %>% filter(is.na(last_t4))
```
...
-   t4_patient_data 환자들 `r nrow(t4_data)`중 처방이 된 환자의 비율은 `r nrow(tx_patient_followup_data)/nrow(tx_patient_data)` 였다.
-   2006년 이후에 치료기록은 있으나 T4 처방이 없는 환자들 `r nrow(tx_patient_no_followup_data)`을 모두 수탁환자라 추정하는 것은 타당해 보인다. 14-04-11 이후의 수탁환자들에 대한 추정을 해보면 `r nrow(tx_patient_followup_data)`
    -   
