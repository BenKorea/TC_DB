...
Tx_Access_raw_list <-my_PtID(Tx_Access_raw)
Tx_Access <-Tx_Access_raw_list$valid_data
Tx_Access_invalid <-Tx_Access_raw_list$invalid_data
...
rm(Tx_Access_raw_list,Tx_Access_invalid)
...
Patient_Tx_Access <- Tx_Access[, .(
  ...
  CumulativeDose = sum(용량, na.rm = TRUE),
  TxNumber = .N,
  Refer = sum(refer, na.rm = TRUE)
), by = .(PtID, 환자이름)]

Patient_Tx_Access_our <- Patient_Tx_Access[Refer == 0]
...
-   그래프를 보면 `r min(Tx_Access$투여일, na.rm=TRUE)`부터이지만, 1996년 이전은 간헐적으로 입력된 것으로 판단되므로, 경우에 따라서는 1996년부터 분석대상으로 하는 것이 고려될 수 있다.
...
Tx_Access_OCS_raw <-bind_rows(Tx_Access,Tx_OCS)

setorder(Tx_Access_OCS_raw, PtID, 투여일)
Tx_Access_OCS_by_PtID_투여일<-Tx_Access_OCS_raw[, `:=`(
...
Tx_Access_OCS_by_PtID_투여일[, remove := fifelse(
...
Tx_Access_OCS_by_PtID_투여일[, 출처 := fifelse(
  ((!is.na(다음투여일_차이) & 다음투여일_차이 < 8) | 
...
Tx_Access_OCS_duplicated <- Tx_Access_OCS_by_PtID_투여일[remove == "Y"]
Tx_Access_OCS <- Tx_Access_OCS_by_PtID_투여일[remove == "N"]
Tx_Access_OCS_Other <- Tx_Access_OCS[치료종류 == "I-131 MIBG" | 치료종류 == "RIT"]
Tx_Access_OCS_RAI <- Tx_Access_OCS[치료종류 != "I-131 MIBG" & 치료종류 != "RIT"]
```

```{r Tx_Access_OCS_RAI, echo=FALSE, message=FALSE, warning=FALSE}

setnames(Tx_Access_OCS_RAI, old = "환자이름", new = "PtName")
setnames(Tx_Access_OCS_RAI, old = "치료종류", new = "TxType")
setnames(Tx_Access_OCS_RAI, old = "투여일", new = "TxDate")
setnames(Tx_Access_OCS_RAI, old = "용량", new = "TxDose")
setnames(Tx_Access_OCS_RAI, old = "회차", new = "TxNumber")
setnames(Tx_Access_OCS_RAI, old = "수탁병원", new = "referH")
setnames(Tx_Access_OCS_RAI, old = "출처", new = "source")
setnames(Tx_Access_OCS_RAI, old = "비고", new = "memo")

```

### OCS와 Access 병합

-   Access 데이터베이스의 `r nrow(Tx_Access)` 건과 OCS 다운로드된 `r nrow(Tx_OCS)` 건을 단순히 병합하면 `r nrow(Tx_Access_OCS_raw)`이지만
...
Tx_EMR_TxDate별 <- Tx_EMR[, .(n = .N), by = .(TxDate)]
setnames(Tx_EMR_TxDate별, old = "TxDate", new = "실시일")
setorder(Tx_EMR_TxDate별, 실시일)  # 오름차순 정렬
plots<-my_daily_graphs(Tx_EMR_TxDate별)
walk(plots, print)
```
...
Tx_EMR <- Tx_EMR[, .SD, .SDcols = c("등록번호","성 명","TxDate","TxType","TxDose","TxNumber","outsideH","TxWBS","TxTg","TxTg_binary")]

setnames(Tx_EMR, old = "등록번호", new = "PtID")
setnames(Tx_EMR, old = "성 명", new = "PtName")
Tx_EMR[,source := "EMR"]
...
- 중복이 되지 않는 치료는 `r nrow(Tx_Access_OCS_EMR)`
로 중복이 되지 않은 치료자료로 진행하였다.
