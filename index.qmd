...
#### ocs_tx 자료의 통합
- D43019의 그래프를 보면 2003년 5월부터 사용이 급감하므로 이전의 자료 `r nrow(D43019_20030501)`를  OCS 기원의 치료자료에 통합하였고
...
acess_tx_patient_our <- access_tx_patient[refer == 0]
acess_tx_patient_refer <- access_tx_patient[refer > 0]
```
...
-   저자는 핵의학치료를 `I-131 치료 정보` 테이블에 2014년7월9일까지 입력했었습니다.
    ...
    -   환자를 기준으로 하면 `r nrow(acess_tx_patient)`명이었고, 입력된 수탁정보를 기준으로
        ...
        -   수탁환자는 `r nrow(acess_tx_patient_refer)`명이었습니다.
