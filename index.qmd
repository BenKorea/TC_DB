...
# # 날짜가 불완전한 경우를 식별 (yyyy-??-??)
# dt[, tx_date_incomplete := str_detect(tx_line, "\\d{4}-\\?\\?-\\?\\?")]
# 
# # 불완전한 날짜와 완전한 날짜를 구분
# dt[, date := str_extract(tx_line, "\\d{4}-\\d{2}-\\d{2}")]
# dt[, date := fifelse(tx_date_incomplete, NA_character_, date)]
# 
# # 불완전한 날짜 레코드와 나머지 레코드 분리
# incomplete_dates <- dt[tx_date_incomplete == TRUE]
# complete_dates <- dt[tx_date_incomplete == FALSE]

# rhTSH 및 MIBG 플래그 추출
dt[, rhTSH := str_detect(tx_line, "\\brhTSH\\b")]
dt[, MIBG := str_detect(tx_line, "\\bMIBG\\b")]
dt[, tx_line := str_remove_all(tx_line, "\\brhTSH\\b|\\bMIBG\\b")]

# 추가 설명 추출 (with ...)
dt[, additional_info := str_extract(tx_line, "\\bwith\\b.*?(?=\\s*\\()")]
dt[, tx_line := str_remove_all(tx_line, "\\bwith\\b.*?(?=\\s*\\()")]

# 병원 이름 추출 (@ hospital_name)
dt[, hospital := str_extract(tx_line, "@\\s*.*$")]
dt[, tx_line := str_remove_all(tx_line, "@\\s*.*$")]

# 용량 정보, 핵심 정보, 세부 정보 추출
dt[, dose := str_extract(tx_line, "\\d+\\s*mCi")]
dt[, primary_info := str_extract(tx_line, "\\bI-131\\b")]
dt[, details := str_extract(tx_line, "\\(.*?\\)")]
dt[, tx_line := str_remove_all(tx_line, "\\d+\\s*mCi|\\bI-131\\b|\\(.*?\\)")]
