...
```{r test}
  ...
  # rhTSH 포함여부에 따라 
  dt[, rhTSH := ifelse(grepl("\\brhTSH\\b", tx_line), TRUE, FALSE)]
  dt[, remained := gsub("\\brhTSH\\b", "", tx_line)]
  
  # with 포함여부에 따라 
  dt[, 치료종류 := ifelse(grepl("\\with\\b", tx_line), TRUE, FALSE)]
  dt[, remained := gsub("\\brhTSH\\b", "", tx_line)]
  
    
  # tstrsplit가 분리되지 않으면 keep=2에서 오류가 발생하므로 분리되는 것만 keep=2
  dt[grepl("@", remained), outside_tx := tstrsplit(remained, "@", fixed = TRUE, keep=2)]
  dt[, remained := tstrsplit(remained, "@", fixed = TRUE, keep=1)]
