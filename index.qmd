...
-   이 때 접수일자기간을 너무 길게 설정하면 오류가 나므로 3개월에서 6개월 정도가 적당해 보입니다.
    그렇지만 특정분기에는 검사 건수가 많아 3개월 단위로 다운로드 하는 것이 제일 안전합니다.
...
을 특정시점부터 사용하고 있습니다.
그래서 감마카메라실 판독소견 다운로드된 자료 중에서 가장 최신 코드인 `HD074` 부터 우선 선별하고 검증을 진행하였습니다.
...
patient_prescription_raw <- merge(patient_tx_access_ocs_emr, prescription_by_pt_id, by = "pt_id", all.x = TRUE)

patient_prescription <- patient_prescription_raw %>% filter(as.IDate(MAX_PRESCRIPTION_DATE - (365+182)) > last_tx)

patient_prescription_last_prescription_is_na <- patient_prescription[is.na(last_prescription),]

patient_prescription$refer_binary <- ifelse(is.na(patient_prescription$last_prescription) & (patient_prescription$last_tx >= MIN_PRESCRIPTION_DATE), TRUE, FALSE)

patient_prescription$followup_loss <- ifelse(is.na(patient_prescription$last_prescription) & (patient_prescription$last_tx < MIN_PRESCRIPTION_DATE), TRUE, FALSE)



summary(patient_prescription)

PRESCRIPTION_DATE_REFER_CRITERIA <- 184



# patient_prescription[, possible_refer := ifelse(
#   is.na(last_prescription), NA, 
#   ifelse(tx_number < 4 & last_prescription - last_tx < PRESCRIPTION_DATE_REFER_CRITERIA & MAX_PRESCRIPTION_DATE - last_tx < PRESCRIPTION_DATE_REFER_CRITERIA,
#          TRUE,
#          FALSE))]
# 
# patient_prescription_possible_refer <- patient_prescription[possible_refer == TRUE,]
# patient_prescription_possible_no_refer <- patient_prescription[possible_refer == FALSE | is.na(possible_refer),]
# 
# summary(patient_prescription_possible_no_refer)
# table(patient_prescription$possible_refer)
```

### 최소 추적관찰기간의 정의
- 갑상선암 재발분석은 최소 추적관찰기간으로써 방사성요오드 치료 이후 재발여부에 대한 검사가 시행되는 것이 6~18개월인 점을 고려하여 `r MAX_PRESCRIPTION_DATE`로부터 18개월 이전의 치료기록까지만의 환자들  `r nrow(patient_prescription)`명을 대상으로 하였습니다.
