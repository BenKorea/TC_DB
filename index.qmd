...
```{r test2}
  ...
  dt[, tx_details := str_remove(tx_details, "[()]")]

  # #\d\. 패턴이 있는 행을 Tx_EMR_error_period로 분리
  Tx_EMR_error_period <- dt[str_detect(tx_details, "#\\d\\.")]

  # 원본 데이터에서 해당 행 제거
  dt <- dt[!str_detect(tx_details, "#\\d\\.")]
  
Tx_EMR_error_period2 <- dt[str_detect(tx_details, "([A-Za-z\\+\\-])\\.")]
dt <- dt[!str_detect(tx_details, "([A-Za-z\\+\\-])\\.")]
  ...
  # 2. 회차 정보만 추출
  dt[, TxNumber := as.integer(str_extract(full_match, "\\d+"))]

  Tx_EMR_error_TxNumber <<- dt[is.na(TxNumber)]
  dt <- dt[!is.na(TxNumber)]

# 1단계: 시작이 ","라면 제거
dt[, tx_details := fifelse(
  str_starts(tx_details, ","), 
  str_remove(tx_details, "^,\\s*"),  # ","로 시작하는 경우 제거
  tx_details
)]
   
 # 2단계: 처음부터 ","를 만날 때까지 추출
dt[, Tx_WBS := str_extract(tx_details, "^[^,]+")]
